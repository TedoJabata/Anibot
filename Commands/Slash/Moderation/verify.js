...
module.exports = {
    ...
    async execute(interaction, client) {
        const role = await interaction.guild.roles.cache.find(role => role.name === config.verifiedRoleName)

        if (!role) {
            await interaction.reply({
                content: 'Verified role doesnt exist! Please create a role named "Verified"',
                ephemeral: true
            })
        ...
        } else if (interaction.member.roles.cache.some(role => role.name === 'Verified')) {
